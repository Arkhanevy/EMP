...
create table clinica( -- tabela com os dados do perifil da clinica
	...
	clin_dtDell datetime,-- data de delete da clinica-- usado para soft delete 
...
create table cliente ( -- tabela do cliente
    ...
    cli_dtDell datetime,-- data de delete
    cli_status varchar(8),-- status do cliente
...
create table servico (
    ...
    ser_status varchar(8),-- status do serviço -- se uma podologa deixar de fazer um serviço dar soft delete para manter o relatorio e atentimento sem dar problema
    ser_dia enum('DOM','SEG','TER','QUA','QUI','SEX','SAB') null,-- dias em que a podóloga realiza tal serviço
    ser_loc int null,-- id da localição no serviço -- vou usar pq tem diferença de tempo de intervalo em diferentes lugares
    constraint fk_ser_pro foreign key (ser_pro) references profissional(pro_id)on delete cascade on update cascade,
    constraint fk_ser_loc foreign key (ser_loc) references localizacao(loc_id)on delete cascade on update cascade
...
create table hr_servico ( -- para cada dia em que a podologa trabralhar vai ter um horario de inicio e terminio e vai servir tbm pra verificar se o horario corresponde ao horario livre
    ...
    hrser_dia enum('DOM','SEG','TER','QUA','QUI','SEX','SAB'), -- dias de trabalho
...
create table agenda (-- aqui vai estar os horarios das consultas
    ...
    agnd_sal int not null,-- id da sala nessa tabela
    agnd_dtIni datetime,-- data de inicio da consulta -- não tem termino pq já vou ter no session a duração e intervalo do servico ent posso calcular sem consultar
    ...
    constraint fk_agnd_loc foreign key (agnd_sal) references sala(loc_id) 
